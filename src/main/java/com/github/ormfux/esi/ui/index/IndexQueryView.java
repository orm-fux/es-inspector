...
public class IndexQueryView extends SplitPane {
    ...
    private BorderPane createQuerySubView(final IndexDetailsController indexController) {
        ...
        searchButton.setAction(() -> {
            ...
            rawResult = searchResult.getResultString();

            if (rawResult != null && rawResult.length() > 3_145_728) { //3MB size limit
                rawResultField.setText("Very large result. Use export to view.");
            } else {
                rawResultField.setText(rawResult);
            }

            treeResultField.setTree(searchResult.getFxTree());
            Platform.runLater(() -> {
                tableResultField.setTableContent(searchResult.getTableData());
            });
        });
    ...
    private AnchorPane createResultSubView() {
        ...
        treeResultTab.setContent(treeResultField);
            ...
            if (targetFile != null) {
                ...
                try {
                    if (fileName.endsWith(".csv") || fileName.endsWith(".CSV")) {
                        ...
                        Files.writeString(targetFile.toPath(), rawResult);
                    }
